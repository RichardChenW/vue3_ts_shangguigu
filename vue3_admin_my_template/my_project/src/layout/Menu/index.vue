<template>
  <!-- <el-menu background-color="#001529" text-color="white" active-text-color="skyblue" > -->
    <template v-for="each in showList" :key="each.path">
      <!--* 没有子路由的情况 -->
      <el-menu-item v-if="!each.children" @click="goRoute" :index="each.path">
        <el-icon>
          <component :is="each.meta.icon"></component>
        </el-icon>
        <template #title>
      ...
      >
        <el-icon>
          <component :is="each.children[0].meta.icon"></component>
        </el-icon>
        <template #title>
